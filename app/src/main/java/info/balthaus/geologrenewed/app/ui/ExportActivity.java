...
import info.balthaus.geologrenewed.app.data.Exporter;
import info.balthaus.geologrenewed.app.data.Exporter.Format;
...
public class ExportActivity extends PreferenceActivity {
    ...
    private void updatePrefs(String key) {
        if ((key == null) || (key.equals(PREF_FORMAT))) {
            ...
            if (val.equals(VALUE_FORMAT_GPX))
                prefFormat.setSummary(formatValue(getString(R.string.export_preference_format_gpx)));
            if (val.equals(VALUE_FORMAT_KML))
                prefFormat.setSummary(formatValue(getString(R.string.export_preference_format_kml)));
        }
