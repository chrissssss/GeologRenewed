...
package info.balthaus.geologrenewed.app.ui;
...
import info.balthaus.geologrenewed.app.data.Database;
...
public class ProfileActivity extends PreferenceActivity {
    ...
    private int getActivityCaption(Database.Activity activity) {
        switch (activity) {
            ...
            case STILL:
                return R.string.profile_preference_caption_still;
            case FOOT:
                return R.string.profile_preference_caption_foot;
            case BICYCLE:
                return R.string.profile_preference_caption_bicycle;
            case VEHICLE:
                return R.string.profile_preference_caption_vehicle;
        }
        return R.string.profile_preference_caption_unknown;
    }
    ...
    private String accuracyToDescription(Database.Accuracy accuracy) {
        switch (accuracy) {
            ...
            case LOW:
                return getString(R.string.profile_preference_accuracy_low);
            case HIGH:
                return getString(R.string.profile_preference_accuracy_high);
        }
        return getString(R.string.profile_preference_accuracy_none);
    }
    ...
    private void updateActivitySettings(Database.Profile.ActivitySettings settings, Preference prefAccuracy, Preference prefLocationInterval, Preference prefActivityInterval) {
        ...
        if (prefLocationInterval != null)
            prefLocationInterval.setSummary(formatValue(settings.getAccuracy() == Database.Accuracy.NONE ? 0 : settings.getLocationInterval()));
    ...
    private void addActivitySettings(PreferenceScreen root, Database.Activity activity) {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                if ((newValue instanceof String) && (((String) newValue).length() > 0)) {
                    ...
                    if (isUnknown) {
                        ...
                        if (enabled) {
                            for (Database.Profile.ActivitySettings a : new Database.Profile.ActivitySettings[]{
                                    profile.getStill(),
                                    profile.getFoot(),
                                    profile.getBicycle(),
                                    profile.getVehicle()
        ...
        switch (activity) {
            case STILL:
                prefActivityIntervalStill = prefActivityInterval;
                break;
            case FOOT:
                prefActivityIntervalFoot = prefActivityInterval;
                break;
            case BICYCLE:
                prefActivityIntervalBicycle = prefActivityInterval;
                break;
            case VEHICLE:
                prefActivityIntervalVehicle = prefActivityInterval;
                break;
        }
    }
